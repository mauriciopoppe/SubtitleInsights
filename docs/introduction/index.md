...
1. **Capture:** Subtitles can be captured automatically from the video player or uploaded manually for analysis.
2. **Translate:** Chrome's Translator API creates natural, context-aware translations for whatever is being said.
3. **Analyze:** Chrome's Prompt API (powered by Gemini Nano) breaks down complex sentences, explaining tricky grammar and cultural nuances.
4. **Present:** Results appear in a clean overlay on the video and a synchronized transcript in the sidebar.
