...
export function SettingsPopup({ isOpen, onClose, triggerRef, platform = 'youtube' }: SettingsPopupProps) {
  ...
  const { aiStatus, warning, isUploadActive, uploadFilename, segments, isAutoGenerated } = useSubtitleStore()
  ...
  const handleIncreaseBatchSize = async () => {
    if (config.autoGeneratedBatchSize < 5) {
      await Config.update({ autoGeneratedBatchSize: config.autoGeneratedBatchSize + 1 })
      setIsRefreshRequired(true)
    }
  }

  const handleDecreaseBatchSize = async () => {
    if (config.autoGeneratedBatchSize > 1) {
      await Config.update({ autoGeneratedBatchSize: config.autoGeneratedBatchSize - 1 })
      setIsRefreshRequired(true)
    }
  }
        ...
        <div className="si-settings-view overlay">
          ...
          {isAutoGenerated && (
            <SettingsItem
              ...
              type="link"
              showArrow={false}
              title={isRefreshRequired ? 'Refresh page to apply changes' : undefined}
              style={isRefreshRequired ? { backgroundColor: 'rgba(255, 214, 0, 0.1)' } : undefined}
              subLabel={
                <div className="si-number-control">
                  <span className="si-number-value">{config.autoGeneratedBatchSize}</span>
                  <div className="si-number-btns-vertical">
                    <button
                      className="si-number-btn-small"
                      onClick={e => {
                        e.stopPropagation()
                        handleIncreaseBatchSize()
                      }}
                      disabled={config.autoGeneratedBatchSize >= 5 || !config.isEnabled || !config.isOverlayEnabled}
                    >
                      +
                    </button>
                    <button
                      ...
                      onClick={e => {
                        e.stopPropagation()
                        handleDecreaseBatchSize()
                      }}
                      disabled={config.autoGeneratedBatchSize <= 1 || !config.isEnabled || !config.isOverlayEnabled}
                    >
                      -
                    </button>
                  </div>
