...
export class SubtitleStore {
  ...
  setIsAutoGenerated(value: boolean) {
  ...
  replaceSegments(newSegments: SubtitleSegment[]) {
    if (!Array.isArray(newSegments)) {
      storeLogger('ERROR: replaceSegments called with non-array:', newSegments)
      return
    }
    this.segments = [...newSegments].sort((a, b) => a.start - b.start)
  ...
  static parseYouTubeJSON(
    json: any,
    batchSize: number = 2
  ): { segments: SubtitleSegment[]; isAutoGenerated: boolean } {
    if (!json.events) return { segments: [], isAutoGenerated: false }

    // Detect if this is an auto-generated subtitle track
    const isAutoGenerated = json.events.some((e: any) => e.segs && e.segs.some((s: any) => 'acAsrConf' in s))
    ...
    // Collapse empty/whitespace segments into the previous non-empty segment
    const collapsed: SubtitleSegment[] = []
    for (const seg of segments) {
      if (seg.text.trim().length === 0) {
        if (collapsed.length > 0) {
          const last = collapsed[collapsed.length - 1]
          // Merge duration but don't append empty/whitespace text
          last.end = Math.max(last.end, seg.end)
        }
        // If there's no previous segment, we just drop it (it's leading whitespace)
      } else {
        collapsed.push(seg)
      }
    }

    const sorted = collapsed.sort((a: SubtitleSegment, b: SubtitleSegment) => a.start - b.start)
    ...
    const unrolled = sorted.filter((s: SubtitleSegment) => s.end > s.start)

    if (isAutoGenerated && batchSize > 1) {
      const grouped: SubtitleSegment[] = []
      for (let i = 0; i < unrolled.length; i += batchSize) {
        const batch = unrolled.slice(i, i + batchSize)
        if (batch.length > 1) {
          grouped.push({
            start: batch[0].start,
      ...
      return { segments: grouped, isAutoGenerated }
    }

    return { segments: unrolled, isAutoGenerated }
  }
}
